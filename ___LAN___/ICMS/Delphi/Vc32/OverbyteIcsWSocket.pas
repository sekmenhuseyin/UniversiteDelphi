...
{$IFDEF USE_SSL}
  ...
  {$IFDEF COMPILER5_UP}
    Contnrs,
  {$ELSE}
    'Bomb SSL requires at least Delphi 5'
  {$ENDIF}
{$ENDIF}
{$IFDEF CLR}
  System.Text,
...
const
  WSocketVersion            = 600;
  CopyRight    : String     = ' TWSocket (c) 1996-2006 Francois Piette V6.00 ';
...
{$ENDIF}
    ...
    procedure   PutDataInSendBuffer(Data : TWSocketData; Len : Integer);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
constructor TCustomWSocket.Create{$IFDEF WIN32}(AOwner: TComponent){$ENDIF};
begin
...
var
    ...
    bMore     : Boolean;
begin
{$IFDEF COMPILER2_UP}                                    { v6.00f }
    EnterCriticalSection(GSendBufCritSect);              { v6.00f }
    try                                                  { v6.00f }
{$ENDIF}                                                 { v6.00f }
    ...
    {$IFNDEF NO_DEBUG_LOG}
                        if CheckLogOptions(loWsockErr) then  { V5.21 } { replaces $IFDEF DEBUG_OUTPUT  }
                            DebugLog(loWsockErr, Name + ' Winsock.Send failed ' + IntToStr(LastError));
    {$ENDIF}
...
{$IFDEF COMPILER2_UP}                                       { v6.00f }
        ...
        LeaveCriticalSection(GSendBufCritSect);             { v6.00f }
    end;                                                    { v6.00f }
{$ENDIF}                                                    { v6.00f }
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TCustomWSocket.PutDataInSendBuffer(Data : TWSocketData; Len : Integer);
var
    oBuffer  : TBuffer;
    cWritten : Integer;
    bMore    : Boolean;
...
begin
    ...
    if IsDigit(Port[1]) then
        ...
        if Pse = IntPtr.Zero then
            raise ESocketException.Create(
                     'WSocketResolvePort: Cannot convert port ''' +
                     Port + ''', Error #' +
                     IntToStr(OverByteIcsWinsock.WSAGetLastError));
...
{$IFDEF CLR}
        ...
        if Ppe = IntPtr.Zero then
            raise ESocketException.Create(
                      'WSocketResolveProto: Cannot convert protocol ''' +
                      sProto + ''', Error #' +
                      IntToStr(OverByteIcsWinsock.WSAGetLastError));
...
{$IFDEF CLR}
    ...
    if FDnsLookupHandle = 0 then begin
        ...
        RaiseException(HostName +
                       ': can''t start DNS lookup, error #' +
                       IntToStr(WSocket_WSAGetLastError));
        Exit;
    end;
{$ENDIF}
{$IFDEF WIN32}
    ...
    if FDnsLookupHandle = 0 then begin
        RaiseException(HostName +
                       ': can''t start DNS lookup, error #' +
                       IntToStr(WSocket_Synchronized_WSAGetLastError));
        Exit;
    end;
{$ENDIF}
...
{$IFDEF CLR}
    ...
    if FDnsLookupHandle = 0 then begin
        FDnsLookupGCH.Free;
        RaiseException(HostAddr + ': can''t start DNS lookup, error #' +
                       IntToStr(WSocket_WSAGetLastError));
    end;
{$ENDIF}
{$IFDEF WIN32}
...
{$ENDIF}
    ...
    if WSocket_Synchronized_bind(HSocket, LocalSockName, SockNamelen) <> 0 then begin
        RaiseException('winsock.bind failed, error #' +
                       IntToStr(WSocket_Synchronized_WSAGetLastError));
        Exit;
    end;
    SockNamelen := sizeof(SockName);
    if WSocket_Synchronized_getsockname(FHSocket, SockName, SockNamelen) <> 0 then begin
        RaiseException('winsock.getsockname failed, error #' +
                       IntToStr(WSocket_Synchronized_WSAGetLastError));
        Exit;
    end;
...
begin
    if FKeepAliveOnOff = wsKeepAliveOFF then  { Defaults to OS setting }
        Exit;
    ...
    if FKeepAliveOnOff = wsKeepAliveOnSystem then
        Exit;
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF CLR}
constructor TCustomSocksWSocket.Create;
begin
    inherited Create;
{$ENDIF}
{$IFDEF WIN32}
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
constructor TCustomLineWSocket.Create{$IFDEF WIN32}(AOwner: TComponent){$ENDIF};
begin
    inherited Create{$IFDEF WIN32}(AOwner){$ENDIF};
    FLineEnd   := #13#10;
    FLineMode  := FALSE;
...
begin
    if FLineMode and (FLineLength > 0) then begin
        ...
        { Move the end of line to beginning of buffer to be read the next time }
        Move(PChar(FRcvdPtr)[BufferSize], FRcvdPtr^, FLineLength - BufferSize);
        {$ENDIF}
        Result      := BufferSize;
    ...
    Move(PChar(FRcvdPtr)[BufferSize], FRcvdPtr^, FRcvdCnt - BufferSize + 1);
    {$ENDIF}
    Result   := BufferSize;
...
{  if (not FLineMode) or (Length(FLineEnd) = 0) then begin }
    ...
    Len := Receive(PChar(FRcvdPtr) + FRcvdCnt, Cnt);
{$IFDEF OLD_20040117}
    if Len <= 0 then
        Exit;
    FRcvdPtr[FRcvdCnt + Len] := #0;
...
{$ENDIF}
    ...
    if Len > 0 then begin
        if FLineEdit then
            ...
            Send(PChar(FRcvdPtr) + FRcvdCnt, Len);
    end;
    ...
    while FLineMode do begin
        ...
        I := SearchFrom;
        while I < (FRcvdCnt - Length(FLineEnd) + 1) do begin
            if PChar(FRcvdPtr)[I] = FLineEnd[1] then begin
                Found := (StrLComp(@(PChar(FRcvdPtr)[I]), @FLineEnd[1], Length(FLineEnd)) = 0);
                if Found then
                    break;    { Found the end of line marker }
            end;
            Inc(I);
        ...
        if FLineLength > 0 then begin
            { Line mode was turned off in the middle of a line read. }
            { We preserve unread line and other received data.       }
            Move(PChar(FRcvdPtr)[I], PChar(FRcvdPtr)[FLineLength],
                 FRcvdCnt - I);
            FRcvdCnt := FRcvdCnt - I + FLineLength;
        end
        else begin
            Move(PChar(FRcvdPtr)[I + Length(FLineEnd)], PChar(FRcvdPtr)[0],
                 FRcvdCnt - I - Length(FLineEnd));
            FRcvdCnt := FRcvdCnt - I - Length(FLineEnd);
        end;
        if FRcvdCnt >= 0 then
            PChar(FRcvdPtr)[FRcvdCnt] := #0;
        SearchFrom := 0;
        { It is possible the user has turned line mode to off. If data is }
        { still available in the receive buffer, we will deliver it.      }
        while (not FLineMode) and (FRcvdCnt > 0) do            { 26/01/04 }
...
initialization
    ...
    InitializeCriticalSection(GSendBufCritSect);            { v6.00f }
{$IFDEF USE_SSL}
...
finalization
    ...
    if WSocketGCount <= 0 then begin    { FP 15/09/03 }
        ...
        DeleteCriticalSection(GSendBufCritSect);            { v6.00f }
    end;
{$IFDEF USE_SSL}
    {$IFNDEF NO_ADV_MT}
