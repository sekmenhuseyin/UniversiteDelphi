...
const
  TIcsWndControlVersion  = 101;
  CopyRight : String     = ' TIcsWndControl (c) 2002-2006 F. Piette V1.01 ';
...
begin
    try
        ...
        // Les exceptions doivent être gérées, sinon l'application sera
        // liquidée dès qu'une exception se produit !
        on E:Exception do
...
begin
    ...
    try
        ...
        // On a une superbe fenêtre. Dans les informations associées, enregistre la
        // référence à notre objet. Elle permettra plus tard d'invoquer la
        // méthode WndProc de gestion des messages envoyés à la fenêtre
...
begin
    ...
    Result.FOwnerList := L;
    OutputDebugString(PChar('Create OwnerList = ' + IntToHex(Integer(Result.FOwnerList), 8)));
...
begin
    ...
    if Index >= 0 then begin
        ...
        if WndHandler.FOwnerList.Count <= 0 then begin
            ...
            if Index >= 0 then
                FList.Delete(Index);
            OutputDebugString(PChar('Free OwnerList = ' + IntToHex(Integer(WndHandler.FOwnerList), 8)));
